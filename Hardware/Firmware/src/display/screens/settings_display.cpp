...
// Settings—Display menu item labels
static const char* SETTINGS_DISPLAY_LABELS[3] = { "Brightness", "Theme", "Home screen" };

static void draw_settings_display_item(uint8_t index, bool is_selected) {
...
void draw_settings_display(uint8_t selection_index) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
        draw_settings_display_item(i, i == selection_index);
    }
...
void draw_settings_display_update_selection(uint8_t old_index, uint8_t new_index) {
    ...
    draw_settings_display_item(old_index, false);
    draw_settings_display_item(new_index, true);
